// KECAP upload method: add 5,576 dummy video samples, each exactly 8 bytes.
...
function patchKecapSampleTableMethod(arrayBuffer) {
  ...
  const realSamples = stsz ? parseStsz(stsz).length : 0;

  const fakePayload = buildKecap8BytePacketsPayload();
  const extraMdat = makeBox('mdat', fakePayload);
  const output = concatBytes([data, extraMdat]);

  return {
    output,
    realSamples,
    fakeSamples: KECAP_8BYTE_PACKET_COUNT,
    fakeSampleSize: KECAP_8BYTE_PACKET_SIZE,
    fakePayloadBytes: fakePayload.length,
    fakeOffset: data.length + 8,
    stcoDelta: 0,
    timelineSafe: true,
  };
}
async function handleSelectedFile(file) {
  ...
  if (selectedFile.size > KECAP_UPLOAD_LIMIT_BYTES) {
    ...
    appendLog('ERROR: Ukuran file melebihi batas 300MB metode KECAP.', true);
    return;
  }
  ...
  try {
    ...
    appendLog('Siap diproses dengan method KECAP.');
  } catch (err) {
    selectedFile = null;
    isResolutionAllowed = false;
...
if (closeBtn) {
  closeBtn.addEventListener('click', () => {
    appendLog('Info: method KECAP ini berjalan 100% di browser, file tidak dikirim ke server.');
  });
}
...
if (patchBtn) {
  patchBtn.addEventListener('click', async () => {
    ...
    appendLog('KECAP method standby...', true);
    appendLog('menambahkan paket dengan mode aman KECAP...');

    try {
      const arrayBuffer = await selectedFile.arrayBuffer();
      const patch = patchKecapSampleTableMethod(arrayBuffer);

      appendLog('Paket 8-byte ditambahkan tanpa mengubah timeline/frame video.');
      appendLog(`Sample video asli terbaca: ${patch.realSamples}`);
      appendLog(`Paket 8-byte ditambahkan: ${patch.fakeSamples}`);
      appendLog(`Ukuran tiap paket dummy: ${patch.fakeSampleSize} byte`);
      appendLog(`Total payload dummy: ${formatBytes(patch.fakePayloadBytes)}`);
      appendLog('Membuat file output anti patah-patah...');
      ...
      a.href = url;
      a.download = `${originalName}_@loweilw_@.ojeeh29.mp4`;
