  ...
  if (!ffmpeg) {
    ...
    ffmpeg = createFFmpeg({
      ...
      progress: ({ ratio }) => {
        ...
        if (percent > 0 && percent < 100 && percent >= lastProgressPercent + 5) {
          lastProgressPercent = percent;
          appendLog(`> FFmpeg processing... ${percent}%`);
        }
      },
    });
  }
...
async function processVideoWithFFmpeg(file) {
  ...
  appendLog('> Menjalankan FFmpeg fast remux MP4...');
  appendLog('> Mode cepat: video/audio copy, bukan re-encode berat.');
  appendLog('> Ini mencegah proses macet lama di 6%.');
  ...
  try {
    ...
    appendLog('> Stream copy gagal, memakai fallback encode cepat...');
    appendLog('> Fallback bisa lebih lama, tunggu sampai selesai.');

    cleanupFs([outputName]);
    lastProgressPercent = 0;
...
if (patchBtn) {
  patchBtn.addEventListener('click', async () => {
    ...
    appendLog('> Mode FFmpeg cepat aktif supaya tidak stuck lama.');
