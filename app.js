const FAKE_SAMPLE_COUNT = 5576;
const FAKE_SAMPLE_SIZE = 8;
const FAKE_SAMPLE_BYTES = new Uint8Array([0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00]);
function buildFakeSamplesPayload() {
  const payload = new Uint8Array(FAKE_SAMPLE_COUNT * FAKE_SAMPLE_SIZE);

  for (let i = 0; i < FAKE_SAMPLE_COUNT; i += 1) {
    payload.set(FAKE_SAMPLE_BYTES, i * FAKE_SAMPLE_SIZE);
  }
...
function patchKecapSampleTableMethod(arrayBuffer) {
  ...
  const mdatPayloadNew = concatBytes([oldMdatPayload, buildFakeSamplesPayload()]);
...
if (patchBtn) {
  patchBtn.addEventListener('click', async () => {
    ...
    appendLog('rebuilding MP4 sample tables...');

    try {
      const arrayBuffer = await selectedFile.arrayBuffer();
      const patch = patchKecapSampleTableMethod(arrayBuffer);

      appendLog('Tabel MP4 berhasil dibangun ulang.');
      appendLog(`Sample video asli terbaca: ${patch.realSamples}`);
      appendLog(`Sample video 8-byte ditambahkan: ${patch.fakeSamples}`);
      ...
      a.href = url;
      a.download = `${originalName}_Kecap_Ojeh.mp4`;
