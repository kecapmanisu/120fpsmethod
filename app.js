...
async function getFFmpeg() {
  if (!window.FFmpeg || !window.FFmpeg.createFFmpeg || !window.FFmpeg.fetchFile) {
    throw new Error('Library FFmpeg belum berhasil dimuat. Pastikan ffmpeg_beautified.js ada di folder web.');
  }
...
if (patchBtn) {
  patchBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      appendLog(`> ERROR: ${err.message}`);
      appendLog('> Jika gagal karena corePath/CORS, jalankan web lewat server lokal atau hosting yang mendukung file WASM.');
    } finally {
